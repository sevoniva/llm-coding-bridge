...
async function status(config, options = {}) {
  ...
  if (["darwin", "win32"].includes(process.platform)) {
    try {
      const service = serviceStatus(options);
      console.log(`[${service.running ? "OK" : "WARN"}] autostart installed=${service.installed} running=${service.running}`);
      if (service.configPath && path.resolve(service.configPath) !== path.resolve(config.path)) console.log(`[WARN] service uses another config: ${service.configPath}`);
    } catch {
      console.log("[WARN] Cannot query autostart. Run llm-coding-bridge service-status.");
    }
  }
  console.log(`logs ${path.join(options.home || os.homedir(), ".llm-coding-bridge", "logs")}`);
