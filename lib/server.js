...
const { createRequestState } = require("./request-state");
const { createRouteHealthRegistry } = require("./route-health");
const { createAdminHandler } = require("./admin");
const { chatClientCompatibility } = require("./chat-client-compat");
const { createUpstreamConcurrencyRegistry, createUpstreamPacingRegistry } = require("./upstream-concurrency");
...
function upstreamForRequest(config, model, req, localToken) {
  let upstream = resolveUpstream(config, model);
  if (!upstream) return upstream;
  if (config.credentialResolver) upstream = { ...upstream, credentialResolver: config.credentialResolver };
  if (upstream.apiKeySource !== "client") return upstream;
...
function startServer(config) {
  ...
  const credentialResolver = config.credentialResolver
    || (config.version === 2 ? createCredentialResolver(config.credentials) : null);
  const normalizedConfig = { ...config, server, credentialResolver };
  const healthRegistry = createRouteHealthRegistry();
  const concurrencyRegistry = createUpstreamConcurrencyRegistry();
  const pacingRegistry = createUpstreamPacingRegistry();
  ...
  const sharedRuntime = Object.freeze({
    ...
    concurrencyRegistry,
    pacingRegistry,
    maxConcurrentRequestsPerProvider: server.maxConcurrentRequestsPerProvider,
    minRequestIntervalMsPerProvider: server.minRequestIntervalMsPerProvider,
    eventStore,
  });
  ...
  const bridgeRuntime = Object.freeze({ ...sharedRuntime, config: normalizedConfig, startedAt });
  const handleAdmin = createAdminHandler({ runtime: bridgeRuntime, localToken, startedAt });
  ...
  function requestRuntime(req, pathname, upstream, clientModel) {
    const context = requestContext(req, pathname, clientModel || "");
    if (!context.requestId) context.requestId = randomUUID();
    return {
      context,
      runtime: {
        ...sharedRuntime,
  ...
  const httpServer = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && pathname === "/health") {
        debug("GET /health");
        if (normalizedConfig.revision) res.setHeader("X-Bridge-Config-Revision", normalizedConfig.revision);
      ...
      if (req.method === "GET" && pathname === "/v1/models") {
        debug("GET /v1/models");
        const data = normalizedConfig.upstreams.map((u) => ({ id: u.alias || u.model, object: "model", created: 0, owned_by: u.name || "upstream" }));
      ...
      if (req.method === "POST" && pathname === "/v1/chat/completions") {
        ...
        const { context, runtime } = requestRuntime(req, pathname, upstream, payload.model);
        const heartbeatIntervalMs = upstream.heartbeatIntervalMs ?? server.heartbeatIntervalMs;
        await handleChat(
          upstream,
          payload,
          res,
      ...
      if (req.method === "POST" && (pathname === "/v1/responses" || pathname === "/v1/responses/compact")) {
        ...
        if (!upstream) return sendJson(res, 404, { error: { message: "Unknown model.", type: "model_not_found" } });
        if (upstream.apiKeySource === "client" && !upstream.clientApiKey) return sendJson(res, 401, { error: { message: "Missing client API key.", type: "auth_error" } });
        const { runtime } = requestRuntime(req, pathname, upstream, payload.model);
        await handleResponses(upstream, payload, res, server.heartbeatIntervalMs, runtime);
        return;
      }
      if (req.method === "POST" && pathname === "/v1/messages") {
        ...
        if (upstream.apiKeySource === "client" && !upstream.clientApiKey) return sendJson(res, 401, { error: { message: "Missing client API key.", type: "auth_error" } });
        const { runtime } = requestRuntime(req, pathname, upstream, payload.model);
        await handleAnthropicMessages(upstream, payload, res, server.heartbeatIntervalMs, runtime);
        return;
      }
  ...
  Object.defineProperty(httpServer, "bridgeRuntime", {
    value: bridgeRuntime,
    enumerable: false,
  });
