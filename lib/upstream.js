...
function upstreamError(message, code, name = "Error", cause) {
  ...
  const boundedBody = new ReadableStream({
    ...
    async pull(controller) {
      try {
        ...
        if (!finished) fail(error?.name === "AbortError" ? error : upstreamError("Upstream response failed.", "UPSTREAM_RESPONSE_FAILED", "Error", error?.cause || error));
      }
    },
    cancel,
  });
...
async function fetchUpstreamJson(upstream, payload, options) {
  ...
  if (!response.ok) {
    await response.text();
    const error = new Error("Upstream HTTP request failed.");
...
async function readUpstreamChatCompletion(response) {
  ...
  if (completion?.error || !Array.isArray(completion?.choices) || completion.choices.length === 0) {
    ...
    const embeddedStatus = Number.isInteger(completion?.code) && completion.code >= 400 && completion.code <= 599
      ? completion.code
      : undefined;
    if (embeddedStatus !== undefined) error.status = embeddedStatus;
    else error.code = "UPSTREAM_NON_SSE_RESPONSE";
    throw error;
  }
  if (normalizedSse) {
