...
const DEFAULT_MAX_CONCURRENT_REQUESTS_PER_PROVIDER = 0;
const DEFAULT_MIN_REQUEST_INTERVAL_MS_PER_PROVIDER = 0;
...
function normalizeServer(server) {
  ...
  const out = {
    ...
    maxConcurrentRequestsPerProvider: numeric(
      "maxConcurrentRequestsPerProvider",
      DEFAULT_MAX_CONCURRENT_REQUESTS_PER_PROVIDER
    ),
    minRequestIntervalMsPerProvider: numeric(
      "minRequestIntervalMsPerProvider",
      DEFAULT_MIN_REQUEST_INTERVAL_MS_PER_PROVIDER
    ),
  };
...
function loadConfig(file) {
  ...
  return Object.freeze({
    path: configPath,
    revision: crypto.createHash("sha256").update(JSON.stringify(config)).digest("hex"),
    server,
...
function resolveApiKey(upstream) {
  if (typeof upstream.apiKey === "string" && upstream.apiKey) return upstream.apiKey;
  ...
  const result =
    typeof command === "string"
      ? process.platform === "win32"
        ? spawnSync(process.env.ComSpec || "cmd.exe", ["/d", "/s", "/c", command], { encoding: "utf8", windowsHide: true })
        : spawnSync("/bin/sh", ["-lc", command], { encoding: "utf8" })
      : spawnSync(command.command, command.args || [], { encoding: "utf8", windowsHide: true });
