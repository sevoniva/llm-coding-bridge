...
async function handleAnthropicMessages(upstream, payload, res, heartbeatIntervalMs, runtime) {
  ...
  try {
    ...
    if (payload.stream) {
      ...
      heartbeat = startHeartbeat(res, heartbeatIntervalMs, {
        onHeartbeat: () => recordHeartbeat(runtime, upstream.alias || upstream.model),
      });
    }
    let chat;
    try {
      const result = await runChatAttempts({
        route: upstream,
        payload: { ...anthropicToChatPayload(upstream, payload), stream: false },
        ...runtime,
        signal: client.signal,
        onJsonCompletion: (value) => { chat = value; },
      });
      chat ||= result.completion;
    } catch (error) {
      ...
      if (payload.stream) {
        if (error.retryable) {
          ...
          await streamAnthropicError(res, error.status ? `Upstream HTTP ${error.status}.` : "Upstream request failed.");
        }
        return;
      }
      const status = error.status || (error.retryable ? 502 : 500);
      sendJson(res, status, { type: "error", error: { type: "api_error", message: error.status ? `Upstream HTTP ${error.status}.` : "Upstream request failed." } });
      return;
    }
    if (heartbeat) heartbeat.stop();
