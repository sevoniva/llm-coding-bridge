...
async function handleAnthropicMessages(upstream, payload, res) {
  ...
  try {
    const upstreamRes = await fetchUpstream(upstream, { ...anthropicToChatPayload(upstream, payload), stream: false }, { signal: client.signal });
    if (!upstreamRes.ok) {
    ...
    if (payload.stream) await streamAnthropic(res, message);
