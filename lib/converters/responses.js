...
async function handleResponsesRequest(upstream, payload, res, signal) {
  const chatPayload = responsesToChatPayload(upstream, payload);
  const upstreamRes = await fetchUpstream(upstream, chatPayload, { signal });
  ...
  try {
    ...
    await upstreamRes.body?.cancel(error).catch(() => {});
    throw error;
  }
  if (!upstreamRes.ok) {
...
async function handleResponses(upstream, payload, res) {
  const client = abortOnClientClose(res);
  try {
    return await handleResponsesRequest(upstream, payload, res, client.signal);
  } finally {
    client.detach();
  }
}
