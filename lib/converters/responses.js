...
const { fetchUpstream, readUpstreamChatCompletion, eachSseData, abortOnClientClose } = require("../upstream");
...
async function handleResponsesRequest(upstream, payload, res, signal, heartbeatIntervalMs) {
  const chatPayload = responsesToChatPayload(upstream, payload);
  const writer = new ResponsesWriter(upstream.model, payload.stream ? res : null, customToolNames(payload.tools), toolSearchNames(payload.tools));
  ...
  const heartbeat = payload.stream ? startHeartbeat(res, heartbeatIntervalMs) : null;
  let upstreamRes;
  try {
    upstreamRes = await fetchUpstream(upstream, chatPayload, { signal });
  } catch (error) {
    if (heartbeat) heartbeat.stop();
    if (!payload.stream) throw error; // preserved: server.js emits HTTP 500
    // Streaming path: headers + response.created already sent, so emit a
    // protocol-correct response.failed rather than tearing down the socket.
    try {
      await writer.fail("Upstream request failed.", "upstream_error");
    } catch {
  ...
  if (!upstreamRes.ok) {
    ...
    const failed = await writer.fail("Upstream error.", "upstream_error");
  ...
  try {
    if (!payload.stream) {
      const data = await readUpstreamChatCompletion(upstreamRes);
      ...
      await eachSseData(upstreamRes.body, async (data) => {
        if (data === "[DONE]") return;
        let chunk;
        try {
    ...
    let failed;
    try {
      failed = await writer.fail("Upstream error.", "upstream_error");
    } catch {
      return;
    }
    if (!payload.stream) sendJson(res, 200, failed);
...
async function handleResponses(upstream, payload, res, heartbeatIntervalMs) {
  const client = abortOnClientClose(res);
  try {
    return await handleResponsesRequest(upstream, payload, res, client.signal, heartbeatIntervalMs);
  } finally {
    client.detach();
  }
}
