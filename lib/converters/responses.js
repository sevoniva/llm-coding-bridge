...
class ResponsesWriter {
  ...
  start() {
    ...
    registerStream(this.res, () => this.fail("Server shutting down.", "server_shutdown"));
...
async function handleResponses(upstream, payload, res) {
  ...
  const upstreamRes = await fetchUpstream(upstream, chatPayload);
  ...
  // If the client (or shutdown) closes res mid-stream, cancel the upstream
  // reader so eachSseData stops feeding deltas to an ended response.
  if (payload.stream) {
    const cancelUpstream = () => upstreamRes.body.cancel().catch(() => {});
  ...
  try {
    if (!payload.stream) {
      ...
      await eachSseData(upstreamRes.body, (data) => {
        if (data === "[DONE]") return;
        let chunk;
        try {
