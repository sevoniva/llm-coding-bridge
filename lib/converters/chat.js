...
async function handleChat(upstream, payload, res, heartbeatIntervalMs, requestLogContext, runtime, clientOptions = {}) {
  const clientModel = upstream.alias || upstream.model;
  const upstreamPayload = {
    ...sanitizeChatPayload(payload, upstream, {
      preserveEmptyAssistantContent: clientOptions.preserveEmptyAssistantContent === true,
    }),
  ...
  try {
    if (!payload.stream) {
      ...
      try {
        const result = await runChatAttempts({
          ...
          upstreamHeaders: clientOptions.upstreamHeaders,
    ...
    // ZCode needs headers committed before an upstream response, while clients
    // with reliable transport attribution can wait and retain the real upstream
    // HTTP status. Both modes use a comment plus an empty completion chunk so
    // comment-aware and data-aware watchdogs observe the same idle activity.
    let streamCommitted = false;
    let heartbeat = { stop() {}, touch() {} };
    const commitStream = () => {
    ...
    if (clientOptions.deferStreamHeaders !== true) commitStream();
    ...
    try {
      await runChatAttempts({
        ...
        upstreamHeaders: clientOptions.upstreamHeaders,
        onHeaders: (response) => {
          logRequestEvent(requestLogContext, "upstream_headers", { status: response.status }, runtime.eventStore);
          const contentType = response.headers.get("content-type") || "";
          if (response.ok && contentType.includes("text/event-stream") && clientOptions.deferStreamHeaders === true) {
            commitStream();
          }
        },
        onData: async (data) => {
      ...
      logRequestEvent(requestLogContext, "upstream_error", { status: error.status, error }, runtime.eventStore);
      if (!streamCommitted && error.status) {
        sendJson(res, error.status, {
          error: {
            message: `Upstream HTTP ${error.status}.`,
            type: "upstream_error",
            ...(error.code ? { code: error.code } : {}),
          },
        });
      } else if (error.retryable) res.destroy();
      else if (!streamCommitted) {
        sendJson(res, 500, { error: { message: "Upstream request failed.", type: "upstream_error" } });
      } else {
        await writeSseErrorAndDone(res, error.status ? `Upstream HTTP ${error.status}.` : "Upstream request failed.", "upstream_error");
      }
      return;
    }
    heartbeat.stop();
