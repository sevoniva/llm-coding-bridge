...
function createCredentialResolver(descriptors, options = {}) {
  ...
  function resolve(reference, request = {}) {
    ...
    if (descriptor.source === "inline") {
      value = descriptor.apiKey;
      if (typeof value !== "string" || !value) throw new Error("Credential is not available.");
    } else if (descriptor.source === "env") {
  ...
  function availability(reference) {
    ...
    return descriptor.source === "command" || descriptor.source === "client" || descriptor.source === "inline";
  }
