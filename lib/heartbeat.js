...
function recordHeartbeat(runtime, model) {
  const heartbeatCount = runtime.requestState.recordHeartbeat();
  try {
    runtime.eventStore?.append({
      type: "heartbeat",
      phase: runtime.requestState.phase,
      requestId: runtime.requestState.requestId,
      model,
      heartbeatCount,
    });
  } catch {}
...
module.exports = { startHeartbeat, recordHeartbeat, DEFAULT_HEARTBEAT_INTERVAL_MS };
