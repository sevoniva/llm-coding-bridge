...
function createAdminHandler(options = {}) {
  ...
  const localToken = typeof options.localToken === "string" && options.localToken.trim()
    ? options.localToken
    : null;
  const runDoctor = typeof options.runDoctor === "function" ? options.runDoctor : null;
  ...
  return async function handleAdmin(req, res, pathname) {
    ...
    if (!runDoctor) {
      sendError(res, 501, "admin_doctor_unavailable", "Doctor action is unavailable.");
      return true;
    }
    try {
      sendJson(res, 200, await runDoctor(body));
    } catch {
