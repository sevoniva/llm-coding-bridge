...
function credentialSummary(route) {
  ...
  if (descriptor.source === "inline") reference = "config-file";
...
function effectiveConfigDocument(config) {
  ...
  return {
    ...
    server: {
      ...
      maxConcurrentRequestsPerProvider: config.server.maxConcurrentRequestsPerProvider,
      minRequestIntervalMsPerProvider: config.server.minRequestIntervalMsPerProvider,
    },
...
function migratedCredential(descriptor) {
  if (descriptor.source === "inline") return { source: "inline", apiKey: descriptor.apiKey };
  ...
  if (descriptor.source === "command") {
    const command = typeof descriptor.command === "string"
      ? process.platform === "win32"
        ? { command: process.env.ComSpec || "cmd.exe", args: ["/d", "/s", "/c", descriptor.command] }
      ...
      ref,
      {
        ...
        reference: descriptor.source === "env"
          ...
          : descriptor.source === "command"
            ? descriptor.command.command
            : descriptor.source === "inline" ? "config-file" : "request",
      },
    ])),
  };
}
