...
function fsyncDirectory(directory) {
  // Windows does not support opening directories through Node's fs.open API.
...
function writePrivateFile(file, data, options = {}) {
  ...
  try {
    ...
    if (process.platform === "win32") runWindows("windows-security.ps1", ["-Action", "ProtectFile", "-File", temporary]);
...
function verifyPrivateRegularFile(file) {
  ...
  if (process.platform === "win32") runWindows("windows-security.ps1", ["-Action", "VerifyFile", "-File", target]);
  else if ((metadata.mode & 0o777) !== PRIVATE_FILE_MODE) throw new Error("Private file mode must be 0600.");
