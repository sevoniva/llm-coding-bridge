...
// Cache the parsed template by mtime because clients poll /v1/models frequently.
...
function codexCatalogModel(upstream, home = os.homedir()) {
  const contextWindow = Number(upstream.capabilities?.contextWindow || upstream.contextWindow || upstream.context_window || 128000);
  const slug = upstream.alias || upstream.model;
  const displayName = upstream.displayName || upstream.name || slug;
  const inputModalities = Array.isArray(upstream.capabilities?.inputModalities)
    ? upstream.capabilities.inputModalities
    : Array.isArray(upstream.inputModalities)
...
function writeChecked(file, text, force) {
  ...
  writePrivateFile(file, text, { exclusive: !force });
}
...
function createCodexProfile(config, name, home, force, options = {}) {
  ...
  const token = config.server.localToken || (upstream.apiKeySource === "client" ? process.env.LLM_CODING_BRIDGE_CLIENT_API_KEY || "<upstream-api-key>" : "local");
  const profile = `model = ${tomlString(clientModel)}
...
`;
  const routes = config.routes || config.upstreams || [upstream];
  writeChecked(catalogPath, `${JSON.stringify({ models: routes.map((route) => codexCatalogModel(route, root)) }, null, 2)}\n`, force);
