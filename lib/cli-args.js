...
const CONFIG_ACTIONS = new Set(["path", "validate", "show", "migrate"]);
const CLIENT_ACTIONS = new Set(["add", "remove", "rollback"]);
const TEMPLATES = new Set(["setup", "config", "codex", "codex-desktop", "claude", "claude-code", "zcode", "z-code"]);
...
const OPTION_DEFINITIONS = Object.freeze({
  ...
  "--from-env": { key: "fromEnv", value: true },
  "--force": { key: "force" },
  "--deep": { key: "deep" },
  "--tools": { key: "tools" },
...
const ALLOWED_OPTIONS = Object.freeze({
  setup: ["--profile", "--advanced", "--yes", ...COMMON_CONFIG],
  "init-files": ["--out", "--home", "--help"],
  reload: COMMON_CONFIG,
  "credential:set": ["--name", "--home", "--from-env", "--help"],
  "config:path": COMMON_CONFIG,
  "config:validate": COMMON_CONFIG,
  ...
  "stop-service": ["--home", "--help"],
  "service-status": ["--home", "--help"],
  "uninstall-service": ["--home", "--help"],
  help: [],
});
...
function commandShape(argv) {
  ...
  if (command === "credential") {
    const action = takeRequiredPositional(argv, "Credential action");
    if (action !== "set") throw new Error("Unknown credential action.");
    result.action = action;
    optionKey = `credential:${action}`;
    ...
    if (client !== "zcode" && !(action === "add" && ["codex", "claude-code"].includes(client))) throw new Error(`Unknown client: ${client}`);
    result.action = action;
...
function validateCombinations(result) {
  if (result.command === "credential" && !result.name && !result.help) throw new Error("credential set requires --name.");
