...
const { loadConfig, normalizeServer } = require("./config");
const { configureClaudeCode, configureCodexDesktop } = require("./client-setup");
...
function validateSetupProfile(input) {
  ...
  if (!Array.isArray(clients) || clients.some((client) => !["zcode", "codex", "claude-code"].includes(client)) || new Set(clients).size !== clients.length) {
...
function loadSetupProfile(file) {
  ...
  let document;
  try {
    document = JSON.parse(fs.readFileSync(resolved, "utf8").replace(/^\uFEFF/, ""));
  } catch {
...
async function runSetup(input, options = {}) {
  ...
  if (profile.service !== "none" && !["darwin", "win32"].includes(platform)) {
    throw new Error("Setup service requires Windows or macOS; choose service none on Linux.");
  }
  const store = options.credentialStore || createCredentialStore({ home, platform });
  for (const model of profile.models) {
    if (model.credential.source === "stored") {
      // Fail before writing config if the saved credential has not been provisioned.
      store.read(model.alias);
      model.credential = store.descriptor(model.alias);
    }
  }
  ...
  if (fs.existsSync(configFile)) {
    ...
    try {
      existing = JSON.parse(sourceSnapshot.bytes.toString("utf8").replace(/^\uFEFF/, ""));
    } catch {
  ...
  normalizeServer(document.server);
  ...
  let zcode = null;
  const clients = {};
  if (profile.clients.includes("claude-code")) clients["claude-code"] = configureClaudeCode(config, home);
  if (profile.clients.includes("codex")) clients.codex = configureCodexDesktop(config, home);
  ...
  if (profile.service === "install") (options.installService || installService)(configFile, { home });
  if (profile.service === "restart") (options.restartService || restartService)(configFile, { home });
...
async function collectSetupProfile(options = {}) {
  ...
  const ask = options.ask || ((question) => askLine(question, { input, output }));
  const platform = options.platform || process.platform;
  const credentialStore = options.credentialStore || createCredentialStore({ home: options.home, platform });
  output.write("LLM Coding Bridge 配置向导 / Setup\nWindows/macOS 会保存密钥；客户端配置会先备份。\n");
  ...
  const models = [];
  for (;;) {
    const alias = (await ask("Client model alias / 客户端模型别名 [coding]: ")) || "coding";
    if (!SAFE_ID.test(alias) || models.some((model) => model.alias === alias)) throw new Error("Setup profile model alias must be a unique safe alias.");
  ...
  const clients = [];
  if (yes(await ask("Configure Codex CLI/Desktop default provider? / 接入 Codex（修改默认模型，先备份）？[y/N]: "))) clients.push("codex");
  if (yes(await ask("Configure Claude Code? / 接入 Claude Code（先备份）？[y/N]: "))) clients.push("claude-code");
  if (yes(await ask("Configure ZCode? / 配置 ZCode？[y/N]: "))) clients.push("zcode");
  const serviceDefault = ["darwin", "win32"].includes(platform) ? "install" : "none";
  const service = (await ask(`Autostart after login (install/none/restart) / 登录自动启动 [${serviceDefault}]: `)) || serviceDefault;
