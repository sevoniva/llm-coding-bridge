...
function configureCodexDesktop(config, home = os.homedir(), options = {}) {
  ...
  fs.writeFileSync(catalogPath, `${JSON.stringify({ models: [codexCatalogModel(config.defaultUpstream, root)] }, null, 2)}\n`);
...
function manualClaude(config) {
  const model = config.defaultUpstream.model;
  const token = config.server.localToken ? "<server.localToken>" : "local";
  return `Claude Code:
  ANTHROPIC_BASE_URL=${localUrl(config)}
