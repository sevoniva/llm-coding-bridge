...
function clientModel(route) {
  return route.alias || route.model;
}

function catalogModels(config, home) {
  return (config.routes || config.upstreams || [config.defaultUpstream]).map((route) => codexCatalogModel(route, home));
}

function claudeEnv(config) {
  const model = clientModel(config.defaultUpstream);
  return {
...
function configureCodexDesktop(config, home = os.homedir(), options = {}) {
  ...
  const backup = backupExisting(file, options);
  writePrivateFile(catalogPath, `${JSON.stringify({ models: catalogModels(config, root) }, null, 2)}\n`);
...
function manualClaude(config) {
  ...
  const token = config.server.localToken
    ? "<server.localToken>"
    : config.defaultUpstream.apiKeySource === "client"
      ? "<upstream-api-key>"
      : "local";
  return `Claude Code:
  ANTHROPIC_BASE_URL=${localUrl(config)}
