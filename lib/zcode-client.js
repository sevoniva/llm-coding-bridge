...
function installedVersion(options = {}) {
  ...
  if ((options.platform || process.platform) === "win32") {
    try {
      const value = runWindows("windows-zcode.ps1", [], { ...(options.run ? { run: options.run } : {}) });
...
function parseJsonSnapshot(snapshot) {
  try {
    return JSON.parse(snapshot.bytes.toString("utf8").replace(/^\uFEFF/, ""));
  } catch {
    return null;
  }
}
...
function detectZcodeState(options = {}) {
  ...
  if (pathEntryExists(configFile)) {
    ...
    try {
      ...
      try { verifyPrivateRegularFile(snapshot.resolvedTarget); privateMode = true; } catch { privateMode = false; }
    } catch (error) {
      snapshot = null;
      document = null;
      previewOnly = true;
