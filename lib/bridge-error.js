...
function parseRetryAfter(value, now = Date.now()) {
  ...
  if (!Number.isSafeInteger(milliseconds) || date.getUTCFullYear() !== numericYear || date.getUTCMonth() !== monthIndex || date.getUTCDate() !== numericDay || date.getUTCHours() !== numericHour || date.getUTCMinutes() !== numericMinute || date.getUTCSeconds() !== numericSecond || date.getUTCDay() !== weekdayIndex || milliseconds <= now) return undefined;
...
function classificationFor(error, status) {
  ...
  const transportClassification = (code) => {
    if (TIMEOUT_CODES.has(code)) return { category: "timeout", retryable: true, scope: "model_route", code: "UPSTREAM_CONNECT_TIMEOUT" };
    if (NETWORK_CODES.has(code)) return { category: "network", retryable: true, scope: "model_route", code: "UPSTREAM_CONNECTION_RESET" };
...
function safeErrorRecord(error) {
  ...
  const own = (key) => Object.hasOwn(source, key) ? source[key] : undefined;
  const record = {
    name: "BridgeError",
