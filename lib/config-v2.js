...
function normalizeBaseUrl(value, name, requireSecure = true) {
  requireNonEmptyString(value, `${name} must be a non-empty URL.`);
  if (CONTROL_CHARACTERS.test(value)) invalid(`${name} must not contain control characters.`);
  ...
  if (requireSecure && url.protocol === "http:" && !isLoopbackHostname(url.hostname)) {
...
function validateLegacyCredential(upstream) {
  if (upstream.apiKey !== undefined) {
    if (typeof upstream.apiKey !== "string" || !upstream.apiKey.trim() || CONTROL_CHARACTERS.test(upstream.apiKey)) invalid("upstream.apiKey must be a non-empty string without control characters.");
    if (upstream.apiKeyEnv || upstream.apiKeyCommand || upstream.apiKeySource) invalid("upstream.apiKey cannot be combined with other credential sources.");
    return;
  }
...
function legacyCredential(upstream) {
  if (upstream.apiKey !== undefined) return { source: "inline", apiKey: upstream.apiKey };
...
function normalizeVersionOne(document, configPath, profilePolicies) {
  ...
  const rawList = Array.isArray(document.upstreams) ? document.upstreams : [];
  ...
  const allRoutes = inputs.map((upstream, index) => {
    ...
    const route = {
      ...
      providerName,
      name: providerName,
      baseUrl: normalizeBaseUrl(upstream.baseUrl, "upstream.baseUrl", false),
      credentialRef,
      credential,
...
function projectCredential(route, credential) {
  if (credential.source === "inline") route.apiKey = credential.apiKey;
...
function validateReferencedCredential(credential) {
  const fail = () => invalid("Referenced credential descriptor is invalid.");
  const keys = Object.keys(credential).sort();
  if (credential.source === "inline") {
    if (keys.length !== 2 || keys[0] !== "apiKey" || keys[1] !== "source") fail();
    if (typeof credential.apiKey !== "string" || !credential.apiKey.trim() || CONTROL_CHARACTERS.test(credential.apiKey)) fail();
    return credential;
  }
  if (credential.source === "env") {
    if (keys.length !== 2 || keys[0] !== "env" || keys[1] !== "source") fail();
    if (typeof credential.env !== "string" || !ENVIRONMENT_VARIABLE_NAME.test(credential.env)) fail();
    return credential;
  }
  if (credential.source === "command") {
    if (keys.length !== 2 || keys[0] !== "command" || keys[1] !== "source") fail();
    if (!isPlainObject(credential.command)) fail();
    const commandKeys = Object.keys(credential.command).sort();
    if (commandKeys.length !== 2 || commandKeys[0] !== "args" || commandKeys[1] !== "command") fail();
    if (
      typeof credential.command.command !== "string"
      || !credential.command.command.trim()
      || CONTROL_CHARACTERS.test(credential.command.command)
    ) fail();
    if (
      !Array.isArray(credential.command.args)
      || credential.command.args.some((arg) => typeof arg !== "string" || CONTROL_CHARACTERS.test(arg))
    ) fail();
    return credential;
  }
  if (credential.source === "client") {
    if (keys.length !== 1 || keys[0] !== "source") fail();
    return credential;
  }
  fail();
}
...
function normalizeVersionTwo(document, configPath, profilePolicies) {
  ...
  for (let providerIndex = 0; providerIndex < document.providers.length; providerIndex += 1) {
    ...
    const providerThinkingTranslation = optionalBoolean(
      provider.translateThinkingToReasoningEffort,
      `${label}.translateThinkingToReasoningEffort`
    );
    const providerMaxOutputTokens = provider.maxOutputTokens === undefined
      ? undefined
      : strictPositiveInteger(provider.maxOutputTokens, `${label}.maxOutputTokens`);
    ...
    for (let modelIndex = 0; modelIndex < provider.models.length; modelIndex += 1) {
      ...
      validateReferencedCredential(credential);
      ...
      const route = {
        ...
        name: providerName,
        baseUrl,
        translateThinkingToReasoningEffort: optionalBoolean(
          model.translateThinkingToReasoningEffort,
          `${modelLabel}.translateThinkingToReasoningEffort`
        ) ?? providerThinkingTranslation ?? false,
        maxOutputTokens: model.maxOutputTokens === undefined
          ? providerMaxOutputTokens
          : strictPositiveInteger(model.maxOutputTokens, `${modelLabel}.maxOutputTokens`),
        credentialRef,
        credential,
...
function normalizeConfigDocument(input, configPath, options = {}) {
  if ((typeof configPath === "object" || typeof configPath === "function") && configPath !== null && types.isProxy(configPath)) {
    invalid("Configuration must contain plain data only.");
  }
  if (typeof configPath !== "string") invalid("configPath must be a primitive string.");
