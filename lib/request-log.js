...
function logRequestEvent(context, phase, details = {}, eventStore) {
  ...
  // The no-store fallback is intentionally limited to request metadata and
  // status. Error objects can carry provider credentials or response content
  // through custom properties; never copy error-derived fields into stderr.
  console.error(`[bridge] ${JSON.stringify(record)}`);
}
