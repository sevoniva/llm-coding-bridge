...
function logRequestEvent(context, phase, details = {}, eventStore) {
  ...
  // The no-store fallback is intentionally limited to request metadata. The
  // details object can carry provider credentials or response content through
  // custom properties, so do not read any of its fields into stderr.
  console.error(`[bridge] ${JSON.stringify(record)}`);
}
