...
async function runChatAttemptsWithoutQueue(options) {
  const {
    route,
    payload,
    requestState,
  ...
  const providerKey = route.providerId || route.baseUrl || alias;
  ...
  while (true) {
    ...
    if (pacingRegistry && minRequestIntervalMsPerProvider > 0) {
      try {
        const paced = await pacingRegistry.wait(providerKey, minRequestIntervalMsPerProvider, signal);
        if (paced.delayMs > 0) {
          appendEvent(eventStore, {
            type: "upstream_rate_wait",
            phase: requestState.phase,
            requestId: requestState.requestId,
            model: alias,
            delayMs: paced.delayMs,
          });
        }
      } catch (rawError) {
    ...
    try {
      const response = await fetchUpstream(attemptRoute, payload, { signal, headers: upstreamHeaders });
...
async function runChatAttempts(options) {
  ...
  if (!concurrencyRegistry || maxConcurrentRequestsPerProvider === 0) {
    return runChatAttemptsWithoutQueue(options);
  }
  const alias = route?.alias || route?.model;
  const providerKey = route?.providerId || route?.baseUrl || alias;
  const queuedAt = now();
  let lease;
  try {
    lease = await concurrencyRegistry.acquire(providerKey, maxConcurrentRequestsPerProvider, signal);
  } catch (rawError) {
    const error = classifyError(rawError, {
      phase: requestState?.phase,
      model: alias,
    ...
    if (requestState) finishState(requestState, error.category === "cancelled" ? "cancelled" : "failed");
    throw error;
  }
  const delayMs = Math.max(0, now() - queuedAt);
  if (lease.queued) {
    appendEvent(eventStore, {
      type: "upstream_queue_wait",
      phase: requestState.phase,
      requestId: requestState.requestId,
      model: alias,
      delayMs,
  ...
  try {
    return await runChatAttemptsWithoutQueue(options);
  } finally {
    lease.release();
  }
}
