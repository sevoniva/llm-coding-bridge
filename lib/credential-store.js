...
const { powershellPath, powershellArgs, runWindows } = require("./windows");
...
function createCredentialStore(options = {}) {
  ...
  function credentialFile(alias) {
    validateAlias(alias);
    // Hex also avoids case-insensitive filesystem collisions on Windows.
    return path.join(home, ".llm-coding-bridge", "credentials", `${Buffer.from(alias).toString("hex")}.dpapi`);
  }

  function descriptor(alias) {
    if (windows) return Object.freeze({ source: "command", command: Object.freeze({
      command: powershellPath(),
      args: Object.freeze(powershellArgs("windows-security.ps1", ["-Action", "Decrypt", "-File", credentialFile(alias)])),
    }) });
  ...
  function save(alias, secret) {
    ...
    if (windows) {
      const encrypted = runWindows("windows-security.ps1", ["-Action", "Encrypt"], { run, input: secretText(secret) });
      if (!encrypted) throw safeError("CREDENTIAL_NOT_AVAILABLE", "Credential is not available.");
      writePrivateFile(credentialFile(alias), encrypted);
  ...
  function read(alias) {
    const reference = descriptor(alias);
    if (windows) return runWindows("windows-security.ps1", ["-Action", "Decrypt", "-File", credentialFile(alias)], { run });
    if (!darwin) {
  ...
  function remove(alias) {
    descriptor(alias);
    if (windows) {
      fs.rmSync(credentialFile(alias), { force: true });
