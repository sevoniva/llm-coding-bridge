...
function emptyContent(content) {
  ...
  return content.length === 0 || content.every((part) => (
    typeof part === "string"
      ? !part.trim()
      : part && typeof part === "object" && ["text", "input_text", "output_text"].some((key) => (
        Object.hasOwn(part, key) && !String(part[key] || "").trim()
      ))
  ));
}

function assistantPayload(message) {
  return ["tool_calls", "function_call", "reasoning_content", "reasoning", "refusal", "audio"]
    .some((key) => {
...
function sanitizeChatPayload(payload, upstream = {}, options = {}) {
  const sanitized = { ...payload };
  if (Array.isArray(payload.messages)) {
    sanitized.messages = payload.messages.flatMap((message) => {
      if (!message || message.role !== "assistant" || !emptyContent(message.content)) return [message];
      if (options.preserveEmptyAssistantContent === true) return [message];
      const { content: _content, ...withoutContent } = message;
      return assistantPayload(withoutContent) ? [withoutContent] : [];
    });
  }
  if (upstream?.translateThinkingToReasoningEffort === true && Object.hasOwn(sanitized, "thinking")) {
    ...
    if (
      ...
      && !Object.hasOwn(sanitized, "reasoning_effort")
    ) {
      sanitized.reasoning_effort = "none";
    }
  }
  const maxOutputTokens = upstream?.maxOutputTokens;
  if (Number.isSafeInteger(maxOutputTokens) && maxOutputTokens > 0) {
    for (const key of ["max_tokens", "max_completion_tokens"]) {
      if (Number.isFinite(sanitized[key]) && sanitized[key] > maxOutputTokens) {
        sanitized[key] = maxOutputTokens;
      }
    }
  }
